...
namespace BookStore.Controllers
{
    public class AccountController : Controller
    {
        ...
        //        if (user != null && !user.IsBlocked && (await _userManager.CheckPasswordAsync(user, loginVM.Password)))
        //        {
        ...
        public async Task<IActionResult> Login(LoginVM loginVM, string returnUrl)
        {
            ...
            {
                var user = await _userManager.FindByEmailAsync(loginVM.EmailAddress);
                if (user != null /*&& !user.EmailConfirmed*/ && !user.IsBlocked && (await _userManager.CheckPasswordAsync(user, loginVM.Password)))
                {
        ...
        public async Task<IActionResult> Register(RegisterVM registerVM)
        {
            ...
            var newUser = new ApplicationUser() { IsBlocked = false, Email = registerVM.EmailAddress, UserName = registerVM.EmailAddress };
        ...
        [HttpPost]
        public async Task<IActionResult> EditUsername(EditUsernameVM model)
        {
            if (ModelState.IsValid)
            {
                var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
                ...
                if (user.UserName != model.NewUserName)
                {
                    ...
                    {
                        await _signInManager.RefreshSignInAsync(user);
                        TempData["Success"] = "Korisničko ime je uspješno ažurirano.";
                ...
                {
                    TempData["Info"] = "Nikakve promjene nisu izvršene.";
                }
            }
