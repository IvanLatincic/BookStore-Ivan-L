...
namespace BookStore.Controllers
{
    public class ProductController : Controller
    {
        ...
        public IActionResult ListOnlyCurrentUsersProducts(string searchString, string minPrice, string maxPrice, int pg = 1)
        {
            string userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            var products = _unitOfWork.Products.GetAllProducts().Where(x => x.UserId == userId);
        ...
        public IActionResult ListUsersProducts(string userId, string searchString, string minPrice, string maxPrice, int pg = 1)
        {
            var products = _unitOfWork.Products.GetAllProducts().Where(x => x.UserId == userId);
            if (!string.IsNullOrEmpty(searchString))
            {
